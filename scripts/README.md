...
| `validate_floor_positioning` | [`validate_floor_positioning.py`](validate_floor_positioning.py) | **DB-direct** floor-position validator (Sprint 66) — run inside the tools-job (`azd-job.sh dev validate_floor_positioning.py -- --tenant-slug demo-wm-dc`). Runs the **production** pipeline (`TimescaleObservationSource` → estimator → `TimescalePositionWriter`) for one tenant against `--emit`-seeded data, then reports computed-vs-placed error + RMSE. `--set-strategy` persists `tenants.position_strategy`; `--clean` removes the seed. Resolves `--tenant-slug` via `uuid5(NAMESPACE_DNS, "<slug>.tagpulse.local")`. |
