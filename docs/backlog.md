...
- [2026-06-21] **Sprint 76 audit — no DB-backed test for the new tag-reads SQL.** `asset_q` (correlated `EXISTS` over active bindings → assets) and `GET /tag-reads/facets` (distinct scheme/antenna) are covered only by the in-memory fake, which can't model the binding join — so the EXISTS correlation and the facet distinct are unverified against a real DB. Same gap as Sprint 70 `tag_q`, and now also the Sprint 77 Transfers/Reconciliation wildcard filters (fake/contract-only). No repo-level DB harness exists (only `tests/integration/test_migration_round_trip.py`). Either stand up an aiosqlite/Testcontainers repo harness or add a focused integration test for the binding-join filter. [backend]
