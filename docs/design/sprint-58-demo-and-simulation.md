...
- Status: **shipped (Phases A–D); Phase E open** (backend PR [#83](https://github.com/9owlsboston/TagPulse/pull/83))
  - Phase D pass bar — "three roadmap items closed" — partially met: API-side baseline captured (`docs/measurements/sprint-58-baseline.md` §2); UI-side stopwatch + Lighthouse rows remain `_TBD_`, so §55.C / §56.B / §57.G stay `[deferred]` in `docs/roadmap.md` with a cross-link to the measurement doc rather than being flipped to `[shipped]` on API numbers alone.
  - Phase E (WM pre-Sprint-59 baseline session) tracked as a Sprint 59 kickoff prerequisite per R5.
...
- **R1 — Simulator drift breaks composition.** If `simulate_devices.py` changes its `--api-key` flag between now and the next sprint, the bundle breaks silently. Mitigation: pin the CLI contracts in `scripts/seed_demo_tenant.py` with explicit args (no `**kwargs` pass-through); cover with one hermetic contract test in `tests/unit/test_seed_demo_tenant.py` that AST-parses each target script's argparse configuration and asserts every `--flag` the composer passes is still declared. Audit-pass note (post-Phase-D): exactly this kind of drift surfaced four endpoint-URL bugs in the Phase D end-to-end smoke (see CHANGELOG entry); the contract test now catches the CLI-flag axis of that risk but does **not** catch endpoint-URL drift inside the scripts themselves — endpoint drift remains a runtime risk only the live `make demo-tenant` exercise covers.
...
- **Q2. Separate API key for continuous simulator vs. seed bundle? — RESOLVED IN PHASE C: SAME KEY.** Phase C kept the working assumption from Phase B — `sim_loop.py` reuses `$TAGPULSE_API_KEY` (the same admin key minted by `make demo-tenant`). Rationale: a second key would have required new KV plumbing on the ACA tools-job path and a parallel rotation workflow in `make demo-tenant`, both for negligible rate-cap-attribution gain (the simulator is the only consumer of the demo tenant outside of operator browser sessions, so its traffic is already trivially attributable). Operational consequence to record: rotating the demo-tenant API key (`make demo-tenant` with `DEMO_KEEP_KEY` unset, or an explicit `--regenerate-key` smoke-setup invocation) requires restarting `sim-start` so the simulator picks up the new key — `make sim-stop && make sim-start` after any rotation.
