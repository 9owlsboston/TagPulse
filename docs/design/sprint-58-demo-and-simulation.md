...
## Open questions to resolve during Phase B

- **Q1.** Should backfilled reads bypass alert-rule evaluation? Working assumption from D3: yes, via `?backfill=true` query param on the ingest endpoint. If that param doesn't exist yet, add it as a Phase-B sub-task (minimal API change, doesn't break the "no new endpoints" constraint).
- **Q2.** Do we need a separate API key for the continuous simulator vs. the seed bundle? Working assumption: same key, simpler. Revisit if rate-cap accounting needs per-source granularity.
- **Q3.** Should the dev Container Apps job emit MQTT (via `mqtt_canary.py`) or only HTTP? Working assumption: HTTP only for now — MQTT path is exercised by the existing canary on a separate schedule and isn't required for demo-tenant fidelity.
