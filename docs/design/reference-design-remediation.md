...
| 2.14 | Tag registry (batch CSV 6 000, reel-range, transfer) | 🟡 | **Reopened — Accepted** ([ADR 028](../adr/028-tags-as-first-class-entity.md)) | TBD | Originally deferred on the basis that bindings + reads could serve a read-only Tags page. SME review surfaced four use cases the bindings-only model cannot answer (owned-but-unread inventory, cross-tenant transfer, stray-read attribution, bulk reel-level lifecycle). ADR 028 introduces `tags` + `tag_transfers` as an identity/ownership layer **above** bindings and reads, with the ingest hot path explicitly unchanged. Five open questions parked for SME / product before implementation begins. |
