...
| 3.3a | Category wiring on Asset CRUD surfaces (Create / Edit modal Select + List column + Detail row + optional list filter) | 🟠 | **Commit** | 37 | Gap surfaced 2026-05-18 while reviewing Sprint 35 doc audit — the `/categories` page exists and the backend `assets.category_id` FK has been live since Sprint 34 (`d6dec19`, row 2.8), but the Asset Create modal, Edit modal, list table, and Detail Descriptions never got the picker. Net effect: users can manage the catalog but cannot actually attach a Category to any asset from the UI. Scope: AntD `<Select>` populated from `useCategories()` in the Create + Edit Asset modals (allowClear; sorted by `category_type` + name); new "Category" column in `AssetList` (renders `category.name` with a `Tag` coloured by `category_type` enum value, sortable by name); new Descriptions row in `AssetDetail` Overview tab; optional `?category_id=` query filter in the list header (parity with Status filter). |
...
| 3.9a | Labels chips on Site / Zone / Device / Category detail pages | 🟠 | **Commit** | 36 (follow-on) | Same `<LabelChips/>` already supports all five `entity_type` values — the work is just wiring it into the four other detail pages (none of them are open yet beyond the existing detail surfaces). One PR. |
| 3.9b | Label filter strip on Assets / Sites / Zones / Devices list pages | 🟠 | **Commit** | 36 (follow-on) | Surface the Phase C `?labels[KEY]=V1,V2` deep-object filter (already live and exercised by tests) as a chip-style filter row above each list table. Includes key-typeahead (from the tenant catalog filtered by `entity_type`) and value-typeahead (from existing values for the selected key). Respects the ADR 020 guard rails: ≤ 5 keys, ≤ 20 values/key. |
| 3.9c | Asset Detail Events Log tab | 🟠 | **Commit** | 36 | New tab on Asset Detail surfacing the existing audit-log + event-bus stream filtered to the asset. Independent of Labels — split out of original row 3.9 once chips shipped. |
| 3.9d | Inline Label picker in Create Asset modal (and equivalents on Site / Zone / Device / Category Create) | 🟠 | **Commit** | 37+ | Gap surfaced 2026-05-18: the `<LabelChips/>` strip ships on Asset Detail (row 3.9 ✅) but only after the entity exists — there's no way to pre-attach labels in the Create flow. Backend doesn't need any change (`POST /{entity_segment}/{id}/labels` already does association; the UI just needs to chain a few of them after the Create response returns). Worth shipping piecemeal as each entity's Create modal is touched, rather than as one big PR. Asset Create rides along with row 3.3a in sprint 37; Site / Zone / Device / Category Create modals pick this up when row 3.9a touches their detail pages. |
