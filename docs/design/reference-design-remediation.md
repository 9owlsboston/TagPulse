...
| 2.3 | Configurable Signaling Events | 🔴 | **✅ Done** ([#54](https://github.com/9owlsboston/TagPulse/pull/54), `e54332b` → `975ef6f`) / 41 | [ADR 021 v2.2](../adr/021-configurable-sensing-events.md) **ratified to Accepted** in Sprint 41 — extends `rules` with 9 additive columns (`event_type`, `trigger`, `processor`, `confidence_threshold`, `category_ids`, `asset_label_filters`, `zone_label_filters`, `site_label_filters`, `integration_ids`) + 12 new `signaling.<event_type>.<trigger>` condition_types + the `?kind={legacy,signaling}` discriminator on the existing `/v1/tenants/{slug}/rules` endpoint (rather than a parallel `/sensing-events` URL — v2.1 deviation, see ADR §Decision history). All four event types ship in migration `040_rules_signaling_events.py`. Phases A schema → B service/cap/dispatcher shell → C envelope → D IsolatedZones + OverlappingZones processors + `on_inference` consumer → E per-rule integration routing + OpenAPI regen all on this PR. Terminology renamed **"sensing" → "signaling"** to align with Azure Monitor vocabulary (ADR 021 v2.1). |
...
| 2.9 | Outbound event envelope (`confidence`, `keySet[]`, `eventConfigurationId`, `categoryId`, `labels[]`) | 🟠 | **✅ Done** ([#54](https://github.com/9owlsboston/TagPulse/pull/54), `feae604`) / 41 | Phase C of [ADR 021 v2.2](../adr/021-configurable-sensing-events.md). New [`src/tagpulse/integrations/signaling_envelope.py`](../../src/tagpulse/integrations/signaling_envelope.py) builds the five additive fields; [`WebhookDispatcher`](../../src/tagpulse/integrations/webhook.py) spreads them once per tick (gated on `Topic.ALERT_TRIGGERED` — raw broadcasts unchanged). Fires for **all** rules: legacy rules get safe defaults (`confidence=1.0`, `keySet=[]`, `eventConfigurationId=str(rule_id)`, `categoryId=null`, `labels=[]`); signaling rules populate from `rules.confidence_threshold` + per-event_type identity keys (`location` → `["asset_id", "zone_id"]`, `geolocation` → `["asset_id", "site_id"]`, `geofencing` → `["asset_id", "zone_id"]`, `temperature` → `["asset_id"]`). Additive — existing webhook consumers see no break. 34 conformance tests in `test_signaling_envelope.py` + `test_webhook_envelope.py`. |
...
| 2.14 | Tag registry (batch CSV 6 000, reel-range, transfer) | 🟡 | **✅ Done** ([ADR 028](../adr/028-tags-as-first-class-entity.md), Sprint 50 Phases A–E, branch `sprint-50/tag-registry`) | 50 | Originally deferred on the basis that bindings + reads could serve a read-only Tags page. SME review surfaced four use cases the bindings-only model cannot answer (owned-but-unread inventory, cross-tenant transfer, stray-read attribution, bulk reel-level lifecycle). ADR 028 introduced `tags` + `tag_transfers` as an identity/ownership layer **above** bindings and reads, with the ingest hot path explicitly unchanged. Shipped: registry CRUD + cross-tenant transfer (Phase B), CSV bulk import with dry-run / confirmation-token / two-person approval / scope-required filters / unified audit log (Phase C, governance §§2–4,7), registrar worker writing `tag_reads.tag_known` (Phase D), reconciliation reports `GET /v1/tenants/{slug}/tags/reconciliation/{view}` with JSON+CSV (Phase E, governance §5). Range-optimization (2.16) explicitly dropped. Phase G validation (registrar integration test + live-PG bulk-flow integration test + reconciliation gauges) deferred to a follow-up sprint. |
...
| 2.16 | Auto-association by reel range | 🟡 | **Drop** | — | Prerequisite (tag registry, 2.14) is now reopened under ADR 028, but auto-association itself remains out of scope — ADR 028 §"Not in scope" explicitly defers it. Manual association is sufficient; revisit only if a customer asks. |
...
| 1.1 | Unify Telemetry/Models/Rules/Alerts → "Signaling Events & Data" | 🟠 | **✅ Done** (TagPulse-UI [#46](https://github.com/9owlsboston/TagPulse-UI/pull/46), [`ebb2f82`](https://github.com/9owlsboston/TagPulse-UI/commit/ebb2f82)) / 41 | Phase F of [ADR 021 v2.2](../adr/021-configurable-sensing-events.md). Sidebar now hosts a **Signaling Events** group (Rules + Alerts under one section) replacing the previous Telemetry/Models/Rules/Alerts spread. The `RuleEditor.tsx` page gains an AntD Tabs wrapper — "Signaling Event" tab is the default for create (with an info Alert pointing at the new pattern) and a **"Legacy rule"** sub-tab keeps the 10 legacy `condition_type` values editable for round-tripping existing rules. The `?kind=signaling` discriminator (Phase B2 backend) drives the list filter. |
...
| 3.5 | Signaling Events modal | 🔴 | **✅ Done** (TagPulse-UI [#46](https://github.com/9owlsboston/TagPulse-UI/pull/46), [`ebb2f82`](https://github.com/9owlsboston/TagPulse-UI/commit/ebb2f82)) / 41 | Phase F of [ADR 021 v2.2](../adr/021-configurable-sensing-events.md). New `src/pages/rules/SignalingRuleModal.tsx` modal walks the operator through: Event Type (Location / Geolocation / Temperature / Geofencing) → Trigger (validated against the 13-pair `SIGNALING_VALID_PAIRS` matrix from `src/types.ts` — UI rejects impossible pairs like `temperature.on_entry` before submit) → per-trigger config form → optional Category / Label scoping. Form POSTs to the unified `/v1/tenants/{slug}/rules` endpoint with `"kind": "signaling"` body — the parallel `/sensing-events` URL from the original reference layout was intentionally not adopted (v2.1 deviation — see ADR §Decision history). 16 vitest cases in `SignalingRuleModal.test.tsx` parametrize all 13 valid pairs + 4 invalid pair rejections. |
