...
TagPulse is the **backend** of a three-repo IoT platform — the admin SPA lives in
[TagPulse-UI](https://github.com/9owlsboston/TagPulse-UI) and the native mobile edge client in
[TagPulse-Mobile](https://github.com/9owlsboston/TagPulse-Mobile), both generated against the same
`openapi.json` contract. It ingests device telemetry —
...
query → alert → integration path is shipped and in production on Azure, through **Sprint 84**
(between sprints as of this snapshot). The most recent arc (Sprints 78–84) built out the
**edge/gateway auth surface** and **mobile enablement**: HTTP **device tokens** (`tpd_`,
least-privilege `device` principals), subject-scoped telemetry **and** external-location ingest,
admin-managed per-gateway **relay grants** (any `subject_kind`), plus VIN/`display_label` asset
lookups the mobile handset uses. The main
in-flight edges are on the MQTT transport: the broker is still Mosquitto on a single ACI
instance with container-local persistence, plaintext `:1883` is still open alongside the
...
- **Device registry & config** — CRUD, per-device profiles, status/last-seen tracking; provision-time HTTP **device tokens** (`tpd_`) mint least-privilege `device` principals. See [docs/data-models.md](data-models.md), [docs/design/device-token-http-auth.md](design/device-token-http-auth.md).
- **Edge / gateway relay auth** — a `device` principal writes its **own** subject by default; an admin can grant a gateway relay authority for a set of `(subject_kind, subject_id)` pairs (`asset`/`device`/`lot`/`stock_item`/`zone`), enforced on both subject-scoped **telemetry** and **external-location** ingest. See [docs/design/gateway-subject-grants.md](design/gateway-subject-grants.md), [docs/design/gateway-grant-relay-extensions.md](design/gateway-grant-relay-extensions.md).
...
- **Mobile edge client** — native iOS/Android field-tech app; API client generated from the same `openapi.json`; scans a VIN and Map-links it to the vehicle asset (`GET /assets/by-binding`). See [TagPulse-Mobile](https://github.com/9owlsboston/TagPulse-Mobile).
...
- **Plaintext `:1883` still open** — the `mosquitto.prod.conf` cutover note originally targeted removal in "Sprint 29", but as of Sprint 84 the plaintext listener is still up alongside `:8883`; removal is effectively **unscheduled**, pending the managed-broker cutover above.
