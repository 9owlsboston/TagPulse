...
- Implements: gap 2.3 (and the outbound-envelope half of 2.9) in `local reference notes/IMPLEMENTATION-GAPS.md`
...
v1 followed `local reference notes/IMPLEMENTATION-GAPS.md` §3.4's
...
- UI gap 1.1 (UI-LOOK-AND-FEEL-GAPS) asks for **one** consolidated
  "Sensing Events & Data" page. One backend table makes that free; two
...
columns. Add new `sensing.<event_type>.<trigger>` `condition_type` values.
...
```sql
ALTER TABLE rules
    -- Sensing-event taxonomy (nullable for legacy rules):
...
1. **`PeriodicSensingDispatcher`** — new worker that wakes on a cadence
   tick and evaluates `sensing.*.periodic` rules. Cadence stored in
   `condition_config.cadence_minutes` (1 ≤ N ≤ 1440). Single new evaluator
   path; no other periodic infrastructure to build.
2. **`sensing.attribution_settled`** — new in-process event bus topic
   (ADR 010) emitted by the OverlappingZones processor when its
   aggregation window resolves. `sensing.*.on_inference` rules subscribe.
3. **Outbound envelope** — `tagpulse/integrations/sensing_envelope.py`
   builds the reference-shaped payload with `confidence`, `keySet[]`,
   `eventConfigurationId` (= `rules.id`), `categoryId`, propagated
   `labels[]`. **This fires for all rules**, not just sensing — legacy
...
- **OverlappingZones** is genuinely new. New module
  `tagpulse/sensing/overlapping_zones.py` runs aggregation window over
  `tag_reads` matching `(asset, [overlapping zones])`, applies RSSI
  floor + time-error filter, weights by aging weight, emits one
  `sensing.attribution_settled` event per zone the asset confidently
...
These resolve to the existing `RuleService` with a `kind=sensing` filter
(rules where `event_type IS NOT NULL`). Reusing `RuleService` keeps RBAC,
RLS, audit, and validation paths consistent. The `/sensing-events`
namespace is for UX — operators see "Sensing Events" not "Rules with
event_type set".
...
- Existing "Rules" sidebar item plus Telemetry / Telemetry Models /
  Alerts are consolidated under a new "Sensing Events & Data" sidebar
  group (UI gap 1.1).
- "Add Sensing Event" modal matches the reference layout (Event Name →
...
- Legacy `condition_type` rules (the 10 existing values) remain editable
  ...
  creation defaults to the Sensing Event flow.
...
- Outbound envelope upgrade benefits *all* rules, not just sensing ones.
...
- Two evaluator wake-up patterns now coexist in the rules engine:
  event-driven (existing + sensing on_change/on_entry/on_exit) and
  cadence-driven (sensing periodic). Bounded by the cadence dispatcher.
...
- Migration `00NN_extend_rules_for_sensing_events.py` adds the columns
...
- No grandfather period needed — legacy rules continue to work
  identically; sensing rules are net-new functionality.
