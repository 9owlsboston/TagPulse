...
- [done, this PR] **H5 — Tests.** Updated 9 unit/integration test files (`test_asset_service.py`, `test_carrier_external.py`, `test_categories.py`, `test_asset_location.py`, `test_sprint21_caches.py`, `test_sprint19_subject_telemetry.py`, `test_label_filter.py`, `test_assets_repository_filters.py`) to drop `asset_type` from fixtures and supply `category_id` where the schema now requires it. Added [tests/unit/test_assets_route_asset_type_removed.py](../tests/unit/test_assets_route_asset_type_removed.py) covering the 400 regression. Migration round-trip is exercised by the existing [tests/integration/test_migration_round_trip.py](../tests/integration/test_migration_round_trip.py) harness (gated on `TAGPULSE_INTEGRATION_DB_URL`).
