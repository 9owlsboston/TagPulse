...
- [planned] **A2 — Partial index** `idx_rules_signaling_active ON rules (tenant_id, event_type, trigger) WHERE enabled = true AND event_type IS NOT NULL` so the new evaluator paths probe a small slice rather than full-scanning `rules`.
- [planned] **A3 — Extend `_RULE_CONDITION_PATTERN`** regex with the 12 new `signaling.*.*` values from [ADR 021 §"New condition_type values"](adr/021-configurable-sensing-events.md). VARCHAR + pattern stays (per ADR open question #1 — easier to extend than Postgres ENUM); the 10 legacy `condition_type` values are untouched.
...
- [planned] **B1 — `RuleService` `kind=` filter** (`signaling` = `event_type IS NOT NULL`, `legacy` = `IS NULL`). API responses expose computed `kind` for clarity; the discriminator stays implicit at the column level (per ADR Consequences §"trade-offs").
...
- [planned] **B3 — `PeriodicSignalingDispatcher`** worker (new module `tagpulse/signaling/periodic_dispatcher.py`). Cadence stored in `condition_config.cadence_minutes` (1 ≤ N ≤ 1440). Wakes on a cadence tick, evaluates `signaling.*.periodic` rules, reuses the existing rules-engine output path (alerts row + integration dispatch).
- [planned] **B4 — `signaling.attribution_settled` event-bus topic** (in-process per ADR 010). Emitted by the OverlappingZones processor when its aggregation window resolves; `signaling.*.on_inference` rules subscribe.
...
- [planned] **C1 — `tagpulse/integrations/signaling_envelope.py`** module that builds the reference-shaped payload: `confidence`, `keySet[]`, `eventConfigurationId` (= `rules.id`), `categoryId` (resolved from the matched entity), `labels[]` (propagated from the matched entity).
- [planned] **C2 — Wire into the dispatcher** so the envelope fires for **all rules**, not just signaling. Legacy rules get safe defaults (`confidence=1.0`, `keySet=[]`, `categoryId=null`, `labels=[]`). Additive — existing webhook consumers see the same payload they get today, plus the five new fields. No behaviour change for legacy rules' historical fields.
- [planned] **C3 — Conformance test** asserting (a) webhook payloads for legacy rules retain every historical field unchanged and gain the five new fields with safe defaults; (b) signaling-rule payloads include populated `confidence` + `keySet[]` + non-null `eventConfigurationId`.

### Phase D — Processor implementations

- [planned] **D1 — IsolatedZones (made explicit).** No algorithm change — the existing implicit single-zone attribution behaviour. New `tagpulse/signaling/isolated_zones.py` codifies it. Default `processor` when NULL and `event_type IN (location, geofencing)`.
- [planned] **D2 — OverlappingZones (genuinely new).** New `tagpulse/signaling/overlapping_zones.py` module per [ADR 021 §"Processor implementation"](adr/021-configurable-sensing-events.md). Runs aggregation window over `tag_reads` matching `(asset, [overlapping zones])`, applies RSSI floor + time-error filter, weights by aging weight, emits one `signaling.attribution_settled` event per zone the asset confidently occupies. Config in `condition_config.processor_config` JSONB (`aggregation_window_s` enum `30 | 60 | 300 | 1800` per ADR open question #5, `min_rssi_dbm`, `zone_bleed_filter`, `aging_weight`, `time_error_filter`).
...
- [planned] **E3 — OpenAPI regeneration** so the TagPulse-UI client picks up the new `?kind=signaling` filter + body schema additions.

### Phase F — UI (TagPulse-UI repo, separate PR)

- [planned] **F1 — "Events & Alerts" sidebar group** consolidating Telemetry / Telemetry Models / Rules / Alerts under one section per the "Unify Telemetry/Models/Rules/Alerts" UI gap 1.1 row in [reference-design-remediation.md](design/reference-design-remediation.md). Label is intentionally **neutral** ("Events & Alerts" — Azure-Monitor-aligned) rather than "Signaling Events & Data" so the post-Sprint-41 taxonomy unification doesn't have to rename it again.
- [planned] **F2 — "Add Alert Rule" modal** per [UI gap 3.5](design/reference-design-remediation.md) and the reference layout — Event Name → Event Type → Category × Labels scoping → Trigger → Confidence → Retention → Connections → Advanced. Form posts to `POST /v1/tenants/{slug}/rules` with `"kind": "signaling"` in the body (per Phase E1 — no `/sensing-events` URL).
- [planned] **F3 — "Legacy rule" sub-tab** keeps the existing rules form editable for the 10 legacy condition types per [ADR 021 §"UI"](adr/021-configurable-sensing-events.md). New rule creation defaults to the Alert Rule flow.
...
- [planned] **F5 — Sidebar reorganization (beyond Events & Alerts).** Audit the current sidebar item ordering and grouping; introduce additional top-level groups so that operational concerns (devices, integrations, configuration) cluster together. Concrete grouping to be locked at PR-time once the F1 group is in; this item ensures the broader navigation is reviewed in the same sprint rather than left to drift again. Includes removing redundant top-level items absorbed into the new groups.
- [planned] **F6 — Collapsible icon-mode sidebar.** Add a chevron toggle that collapses the sidebar to an icon-only rail (Ant Design `Sider` `collapsible` + `collapsedWidth={64}` pattern) and expands it back. Goal: reclaim horizontal real estate for wide tables (Telemetry, Rules, Tag Reads, Audit). Persist the collapsed state in `localStorage` per tenant + per user. Tooltips on hover for the icon-only state so navigation stays discoverable.
- [planned] **F7 — Asset form: drop `Type` field.** Removes the `asset_type` input from Asset create/edit forms; Category becomes the sole classifier (per Phase H closing ADR 019). Filter chips on the Assets list page drop the legacy Type chip in favour of the existing Category chip.
- [planned] **F8 — vitest coverage** of the collapsed-sidebar interaction (toggle, persistence, tooltip visibility) and a smoke test that the reorganized menu renders all routes accessible.

### Phase G — Documentation + roadmap reconciliation

- [planned] **G1 — Ratify [ADR 021](adr/021-configurable-sensing-events.md)** Proposed → **Accepted**. Update its revision history with the Sprint 41 merge SHA + note the v2.1 terminology rename (sensing → signaling).
- [planned] **G2 — Flip remediation rows** [2.3](design/reference-design-remediation.md) (Configurable Signaling Events backend) + [2.9](design/reference-design-remediation.md) (Outbound event envelope) + the [§3.2 1.1](design/reference-design-remediation.md) "Unify Telemetry/Models/Rules/Alerts" row + [3.5](design/reference-design-remediation.md) ("Add Alert Rule modal") to **✅ Done** with the implementing merge SHAs, per the doc's own §7 "Updating this document" rule.
- [planned] **G3 — User-guide section "Configurable Signaling Events"** walking through one rule per event type — Location `on_change`, Temperature `periodic`, Geofencing `on_entry`, Geolocation `on_inactivity` — with the four matching curl recipes (POST to `/rules` with `"kind": "signaling"` body).
- [planned] **G4 — CHANGELOG `## Unreleased`** entry summarising the ADR 021 v2 ratification, the five new additive envelope fields, the `assets.asset_type` removal (Phase H, breaking), and the sidebar reorganization + collapsible icon-mode (Phase F).
- [planned] **G5 — ADR 019 close-out.** Flip [ADR 019](adr/019-categories.md) Status → **Completed** with the Sprint 41 merge SHA. Update [docs/data-models.md](data-models.md) `assets` table (drop the `asset_type` row + the deprecated-shadow note) and [docs/design/assets-and-zones.md](design/assets-and-zones.md) + [docs/design/tracking-modes.md](design/tracking-modes.md) + [docs/guides/domain-concepts-101.md](guides/domain-concepts-101.md) to remove the `asset_type` field from schema snippets and example payloads. Remediation row for ADR 019 (if any open) flips ✅ Done.

### Phase H — Close ADR 019: drop `assets.asset_type`

ADR 019 (Sprint 34) introduced `category_id` as the structured replacement for the free-form `assets.asset_type` `VARCHAR(50) NOT NULL` shadow column, with the explicit plan: "Drops in a future migration once UI + clients have switched." Sprint 41 is that migration. The new Signaling Events flow scopes by category, not type, so completing the deprecation here keeps the API surface internally consistent. **Breaking API change** — bumps the Unreleased section with a migration note for external webhook consumers and any direct API callers.

- [planned] **H1 — Backfill + NOT NULL migration.** Alembic migration that (a) `UPDATE assets SET category_id = <default-category-for-tenant>(asset_type) WHERE category_id IS NULL`, (b) `ALTER TABLE assets ALTER COLUMN category_id SET NOT NULL`, (c) `ALTER TABLE assets DROP COLUMN asset_type`. Idempotent against partial reruns. Drops the matching index `ix_assets_tenant_type` if present (per [docs/design/assets-and-zones.md](design/assets-and-zones.md)).
- [planned] **H2 — Pydantic schemas.** Remove `asset_type` from `AssetCreate` / `AssetUpdate` / `AssetResponse` / `AssetInZoneSummary` / any other `Asset*` schema in [src/tagpulse/models/schemas.py](../src/tagpulse/models/schemas.py). `category_id` becomes required on `AssetCreate` (was nullable during the ADR 019 compatibility window).
- [planned] **H3 — `AssetService` + repos.** Remove the `asset_type` parameter from `list_assets()`, the `AssetModel.asset_type` filter branch in [src/tagpulse/repositories/timescaledb/assets.py](../src/tagpulse/repositories/timescaledb/assets.py), the raw-SQL `asset_type` columns in [src/tagpulse/repositories/timescaledb/asset_location.py](../src/tagpulse/repositories/timescaledb/asset_location.py), and the `asset_type` key from the audit-log `changes` dict in [src/tagpulse/api/services/asset_service.py](../src/tagpulse/api/services/asset_service.py).
- [planned] **H4 — API surface.** Remove `?asset_type=` query param from `GET /assets`; return **HTTP 400** with a hint pointing at `?category_id=` for one release (then drop the hint). Update [openapi.json](../openapi.json) via the regen pipeline.
- [planned] **H5 — Tests.** Update all fixtures and unit/integration tests that construct `Asset*` Pydantic models or POST asset payloads to drop the `asset_type` field. Add a migration test asserting backfill correctness against a synthetic fixture with one NULL `category_id` row per legacy `asset_type` string. Add an API-level regression test that `GET /assets?asset_type=foo` returns 400 with the migration hint.
- [planned] **H6 — EPC parser stays.** The `asset_type` *bits* parsed from EPC tag binary in [src/tagpulse/rfid/epc.py](../src/tagpulse/rfid/epc.py) are RFID protocol semantics, not the DB column; that code is untouched. Rename the local variable to `epc_asset_type_bits` only if it improves clarity — no behaviour change.
