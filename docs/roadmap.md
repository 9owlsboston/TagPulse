...
**Current sprint:** 84 — grant relay extensions · **shipped** (PR #181); between sprints.
<!-- current-sprint:end -->
...
- [done] **E3 — CI gating on migration round-trip (C-EKF0).** A `migration-check` job in `.github/workflows/ci.yml` runs `make migration-check` (Alembic upgrade head → downgrade -1 → upgrade head) against a `timescale/timescaledb:latest-pg16` service container, blocking merge on a broken migration downgrade. The `-1` step validates the newest migration each PR (a new migration is always `head` in its introducing PR). Closes the Sprint 19 carry-over flagged in the cloud-readiness review.
...
- [docs/refs/docs-audit-sprint28.md](refs/docs-audit-sprint28.md) exists and is linked from `docs/roadmap.md` (this entry), and every gap in it either ships in Sprint 28 or has an explicit "deferred — gated on…" line.
...
- Auto-association by reel range (gap 2.16) — explicitly dropped; manual association suffices.
- Rigid hierarchy above `batch` (reel → shipment → PO → supplier) — operators use labels.
- Tenant-level "auto-register on first read" opt-in setting — deferred until a customer asks.
- Tag credentials / Gen2v2 authenticate — Backlog.

**Risks / open items entering the sprint:**
- **Registrar worker lag SLI: p95 < 10 s** (locked at planning). Surface as an explicit SLI before merging; `tag_known=NULL` UX copy assumes operators effectively never see it. Worker sizing in Phase D1 must hit this on representative ingest rates.
- **Reserved-label-key collision policy: refuse + manual intervention** (locked at planning). The A3 migration must detect any pre-existing `batch` (or other reserved `batch.*`) key in a tenant's labels catalog and **fail the migration for that tenant with a structured error** pointing the operator at a rename runbook. No auto-rename, no silent coexistence — operator must consciously reconcile before tag registry rolls out for that tenant. Runbook lives in [docs/runbooks/](runbooks/) (drafted in F2).

---

## Sprint 51 — Tag registry UI (shipped — surfaces [ADR 028](adr/028-tags-as-first-class-entity.md) backend in [TagPulse-UI](https://github.com/9owlsboston/TagPulse-UI))

**Goal.** Make the Sprint 50 tag registry operator-usable from the admin UI. Sprint 50 shipped the full backend (`tags`, `tag_transfers`, `pending_bulk_operations`, registrar worker, reconciliation reports, CSV import, two-person rule) — every workflow ADR 028 contemplates is reachable today via cURL against the documented routes in [runbooks/tag-registry-operations.md](runbooks/tag-registry-operations.md), but no screen exists. ADR 028 §"Negative / costs" called this out at planning time: *"Two new admin workflows (CSV import, transfer) need UI before the registry is operator-usable. Backend can ship first; UI is a separate sprint."* This is that sprint. **No backend changes expected** — if a UI flow surfaces an API gap, scope-creep it into a Sprint 51b backend follow-up rather than mixing repo PRs.

**Cross-repo coordination.** Implementation lands in the [TagPulse-UI](https://github.com/9owlsboston/TagPulse-UI) repo (React + AntD, same stack as the existing admin surfaces). This roadmap entry tracks the work from the backend side: API contract stability, OpenAPI client regeneration, runbook updates, and the ADR 028 v1.2 entry flipping the "UI deferred" caveat to "shipped in Sprint 51". UI PRs cross-link back to this entry; backend-side commits in TagPulse stay limited to docs + (if needed) `openapi.json` regeneration with zero signature drift.

**Phases.**

- **A — Design.** [shipped — [TagPulse#69](https://github.com/9owlsboston/TagPulse/pull/69)] UI design doc in `docs/design/tag-registry-ui.md` (this repo, so the design rationale lives next to ADR 028): information architecture (top-level "Tags" nav entry alongside "Assets" and "Devices"; sub-routes `/tags`, `/tags/:epcHex`, `/tags/import`, `/tags/transfers`, `/tags/reconciliation/:view`), wireframes for the four flows (list/detail, CSV import wizard, transfer request + approval, reconciliation views), component inventory mapping each screen to the existing AntD primitives in TagPulse-UI (Table, Steps, Drawer, Modal, Segmented), copy deck for the operator-facing strings that need to match runbook vocabulary (`status` enum names, "two-person approval", `tag_known` lag note). Pin the **`tag_known=NULL` UX policy** — never surface NULL as a visible state in filters or columns; treat it as "loading" with the registrar SLI as the implicit promise (p95 < 10s per the Sprint 50 risk lock).
- **B — Tag list + detail.** [shipped — [TagPulse-UI#55](https://github.com/9owlsboston/TagPulse-UI/pull/55)] Tag list page (server-side pagination via the existing `GET /v1/tenants/{slug}/tags` `?status`/`?labels[batch]=`/`?epc_prefix`/`?bound` filters; column set: EPC hex, status badge, source, batch label (resolved from `entity_labels`), `last_seen_at` relative time, bound asset/stock_item link if any). Tag detail drawer / page with status lifecycle history (audit-log filter: `?subject_type=tag&subject_id={id}`), inline status transition (admin-only; per-row PATCH respects `validate_status_transition`), label editor (consumes the existing labels API — reserved-key guard from the Sprint 50 post-ship audit already enforces this server-side, UI shows a disabled / tooltipped row for `batch.*` keys to explain why).
- **C — CSV import wizard.** [shipped — [TagPulse-UI#60](https://github.com/9owlsboston/TagPulse-UI/pull/60) (replaces auto-closed #56)] Multi-step wizard (AntD `Steps`): (1) upload + parse client-side preview, (2) submit as `?dry_run=true` → render `TagBulkOperationResult.sample` + per-row errors from `TagBulkRowError[]`, (3) confirm via `?confirm=<token>` — when the response is 202 `requires_approval=True`, route the operator to the pending-approval landing page (Phase F surface) instead of celebrating success. Surface the 8 MiB / 10 000-row / 10-per-hour limits up front so operators don't discover them on submit. Operator copy for the two-person threshold sourced from the resolved tenant config (`tenants.tag_bulk_two_person_threshold`).
- **D — Cross-tenant transfer flow.** [shipped — [TagPulse-UI#57](https://github.com/9owlsboston/TagPulse-UI/pull/57)] Two-tenant Admin role required (server enforces this — UI shows a disabled CTA + tooltip listing the missing role when the current user lacks Admin on the destination tenant). Request form binds against `POST /v1/tenants/{slug}/tag-transfers` (sender side); approval queue page on the receiving tenant lists pending inbound transfers with the Sprint 50 dry-run sample shape. Cross-link both sides via `request_id` (already a first-class column on `audit_logs` since Phase C5).
- **E — Reconciliation views.** [shipped — [TagPulse-UI#58](https://github.com/9owlsboston/TagPulse-UI/pull/58)] Three new operator pages backed by `GET /v1/tenants/{slug}/tags/reconciliation/{view}` (viewer role — visible to anyone with API access, no admin gate): `registered-unread`, `unregistered-reading`, `bindings-on-retired`. Each view is a paginated table with `?days=` selector (default 30; bounded 1–365 by the backend) and a "Download CSV" button that hits `?format=csv` — matches the runbook's spreadsheet-binding contract. No mutation surfaces on these pages by design (read-only triage views per Phase E's contract).
- **F — Pending-approval inbox.** [shipped — [TagPulse-UI#59](https://github.com/9owlsboston/TagPulse-UI/pull/59); backend list endpoint in Sprint 52] Single wizard surface (label-batch scope OR explicit EPC list ≤ 1000, dry-run preview, confirm or escalate-to-approval) backed by `POST /v1/tenants/{slug}/tags/bulk-patch` and `bulk-retire`. Pending-approval inbox page at `/admin/pending-bulk-operations` consuming the existing `bulk_operations.py` admin endpoints — approve / reject buttons gated on the second-admin rule (UI must hide both when `current_user.id == row.requested_by`; the server already 403s this, the UX should not let the operator hit the wall). Re-used by Phase C (imports) and Phase D (transfers above threshold) — the inbox is a single landing page for all three pending-op types, distinguished by the `operation` column.
- **G — Docs + ADR linkage.** [partial — this PR flips Sprint 51 entry to `(shipped)`; ADR 028 v1.2 status update + runbook "Operator UI quick reference" section deferred to a follow-up] ADR 028 status flipped Implemented (Sprint 50) → **Implemented + UI shipped (Sprint 51)** with a v1.2 decision-history entry citing the UI PR set. [runbooks/tag-registry-operations.md](runbooks/tag-registry-operations.md) gets a new top section "Operator UI quick reference" mapping each runbook section to its new UI screen — the cURL examples stay (operators automating against the API still need them) but the recommended path becomes the UI for day-to-day operations, mirroring the [docs/guides/domain-concepts-101.md](guides/domain-concepts-101.md) §"Everything in this guide is doable from the UI" convention. This entry flipped to `(shipped)` with the cross-repo PR list.

**Out of scope for Sprint 51:**
- Backend API changes — if a UI flow surfaces an API gap, scope it into Sprint 51b. Sprint 51 is a UI-only sprint by construction so the OpenAPI surface stays a stable contract for the duration.
- "Tag registry overview" dashboard widget on the home page — defer until operators have lived with the dedicated `/tags` surface for a sprint and we know which counters they actually want at-a-glance.
- Tag credentials / Gen2v2 authenticate UI — backend deferred to Backlog (ADR 028 §"Not in scope"), no UI to build.
- Auto-association by reel range, range-based batch optimization, rigid hierarchy above `batch` — Sprint 50 §"Out of scope" carries these over unchanged; UI inherits the deferral.
- Bulk-op audit log filter UI (`?request_id=` / `?batch=` on `/admin/audit-logs`) — backend shipped this in Phase C5 but the existing admin audit-log page (`src/pages/admin/AuditLog.tsx` in TagPulse-UI) needs its own filter-surface refresh; carry to a follow-up because it touches the audit page's preset architecture rather than the new `/tags` surface.

**Risks / open items entering the sprint:**
- **Cross-repo PR sequencing.** UI PRs depend on the backend `openapi.json` shipped in Sprint 50. Lock the OpenAPI client version pinned in TagPulse-UI to the Sprint 50 release tag at sprint kickoff so a mid-sprint TagPulse change can't churn the generated TypeScript client out from under in-flight UI work. If a backend gap appears, ship the fix as Sprint 51b on this repo and bump the UI pin in a single coordinated commit.
- **Pending-approval inbox already needs to multiplex three operation types** (`tags.import`, `tags.bulk_patch`, `tags.bulk_retire`, `tag.transfer`) from day one. The risk is building a per-op inbox by mistake — design the inbox in Phase A around the existing `pending_bulk_operations.operation` discriminator so adding the future Sprint 51+ device bulk ops is a renderer registration, not a page rewrite.
- **`tag_known=NULL` UX policy.** Phase A must explicitly resolve how the UI renders reads in the brief window before the registrar worker classifies them. Two viable choices: (a) hide NULL rows from default filters (registrar SLI promise carries the contract — at p95 < 10 s this is invisible to operators), or (b) badge them as "Pending classification" with a tooltip. Pick one in Phase A, document in [docs/design/tag-registry-ui.md](design/tag-registry-ui.md), don't punt — the answer informs every list/detail screen's empty-state copy. **Resolved in Phase A:** option (a) — registry list is over the `tags` table (registry rows), reads in the brief unclassified window appear nowhere by design; `unregistered-reading` reconciliation view filters them out per Phase E's lag-induced-false-positive avoidance.

---

## Sprint 52 — `GET /bulk-operations` list endpoint (shipped — [PR #71](https://github.com/9owlsboston/TagPulse/pull/71); closes Sprint 50 Phase C3 deferred surface)

**Goal.** Close the one API gap surfaced by Sprint 51 Phase F (admin pending-bulk-operations inbox UI). Sprint 50 Phase C3 shipped the `pending_bulk_operations` substrate + per-id `GET / approve / reject` endpoints but deliberately deferred the list surface — the route module's own docstring carried the caveat *"The list endpoint (`GET /bulk-operations?status=pending`) is intentionally out of scope for C3 — operators query `pending_bulk_operations` directly via the admin UI's existing audit-log surface until the C5 unified shape lands."* C5 landed in Sprint 50 and the Phase F UI inbox at `/admin/pending-bulk-operations` needs a typed list surface, so this sprint closes that gap.

**Shipped.**

- **New endpoint:** `GET /v1/bulk-operations` (admin or editor, tenant-scoped via the standard `require_role` + RLS plumbing) returns the per-tenant inbox of `pending_bulk_operations` rows ordered `created_at DESC`. Query params: `?status` (validated as `^(pending|approved|rejected|executed|expired)$` — 422 on typo so operators get a real signal instead of an empty list), `?operation` (1..64 chars, free-form filter on the operation discriminator — `tags.import` / `tags.bulk_patch` / `tags.bulk_retire` today, future bulk ops without route changes), `?limit` (1..500, default 100), `?offset` (>=0, default 0). Response is `list[PendingBulkOperationResponse]` — the **existing** schema from Phase C3, which already omits the raw `payload` bytes (operators inspect via the `sample` first-10-EPCs field).
- **New service helper:** `pending_ops.list_pending(session, tenant_id, *, status=None, operation=None, limit=100, offset=0)` in [src/tagpulse/services/pending_bulk_operations.py](../src/tagpulse/services/pending_bulk_operations.py) — pure-async, mirrors the existing `_load` / `create_pending` / `approve` / `reject` signatures.
- **Tests:** 5 route-layer cases in new [tests/unit/test_bulk_operations_list.py](../tests/unit/test_bulk_operations_list.py) + 3 service-layer cases appended to [tests/unit/test_pending_bulk_operations.py](../tests/unit/test_pending_bulk_operations.py). `make check` clean: 1367 passed, 1 skipped (+8 from Sprint 50 close-out's 1359).
- **Downstream consumer:** [TagPulse-UI#59](https://github.com/9owlsboston/TagPulse-UI/pull/59) Phase F admin inbox at `/admin/pending-bulk-operations` per [docs/design/tag-registry-ui.md §7.2](design/tag-registry-ui.md).

**Out of scope:** auto-expiry of stale pending rows (the `expired` status is a queryable filter but no worker writes it — defer until operator demand exists), webhook fanout on pending-op creation (Sprint 8 outbound webhooks can already subscribe to `audit_logs` rows for this if needed), and bulk-approve / bulk-reject endpoints (operators triage one row at a time in the UI; the per-id endpoints suffice).
...
- **F — Test-client v2 wire-format catch-up.** [done, this PR] Sprint 46/47 shipped the v2 wire format (ADR 025/026/027) on the production ingest path but left the operator-facing test clients on v1, so manual smoke runs and the production canary never exercised the v2 dispatch + presence-reconciler branch. **F1**: [`clients/pi/examples/paho_smoke_publisher.py`](../clients/pi/examples/paho_smoke_publisher.py) gains a `--wire v2` mode with `--v2-type {snap,appeared,disappeared}` and `--sn` flags emitting inline `t=0/1/2` payloads that mirror [`WmV2Producer`](../clients/pi/tagpulse_edge/wm_v2_producer.py); validates EPCs client-side to match the spec §6 `invalid_epc` reason. **F2**: [`scripts/mqtt_canary.py`](../scripts/mqtt_canary.py) (Sprint 28 C2 in-VNet canary used by the D2 alert and the C4 outage runbook) now publishes a v2 `t=1` appeared message with a per-run 24-hex-char EPC `CA<…>`, polling `tag_reads` by EPC so it proves broker → subscriber → v2 dispatch → presence reconciler → ingestion → DB end-to-end on every 5-minute run. Audit row #11 sub-item (refactor `clients/pi/examples/run_reader.py` off the legacy `EdgeAgent`/`RawTagRead`/`SensorSample`/`LocationFix` shapes) and wiring out `tests/conformance/` remain deferred — both are larger `clients/pi` surface-area cleanups better handled as a dedicated Sprint 53 Phase G follow-up.
...
> **Status (2026-06-14, post-remediation).** Backend mechanism shipped (ADR-032 §7 steps 1–5, PR [#91](https://github.com/9owlsboston/TagPulse/pull/91); [ADR-032](adr/032-configurable-ui.md) **Accepted**). A gap audit then drove a remediation pass that closed the consumption + write-surface gaps: **the UI now consumes all six leaves** — `labels` (`Device`→`Reader`), `nav` (`applyNavConfig`), `cards` (`cards.dashboard` default beneath localStorage), `theme` (`<html>` data-attrs), **`columns`** (`applyColumnConfig` — TID + User Memory advanced on Tag Reads, Registered advanced on Assets) and **`tables`** (`applyDefaultSort` — config-driven default sort). The previously-missing **write surface** landed as a **`Preferences` page** (`PUT /ui-config/me` — hide dashboard cards cross-device + "Reset to team default"). The **§59.8 Units table** (carried from Sprint 59) shipped on Product detail (per-unit list + zone/state filters + movement-history drawer). The full **WM persona** is now seeded onto the demo tenant from the canonical `WM_DEMO_PRESENTATION` (label skin + nav-simplification + hidden cards + sparkline + advanced columns + newest-first sort), validated to resolve through the real schema. **What still remains (tail):** (1) an **in-app admin UI** to edit tenant/role defaults — the `PUT /ui-config/{tenant,role}` endpoints exist and are seed-driven today, but there is no admin screen; (2) the **`locked` leaf-pin** (ADR-032 §2, deliberately deferred until floor layers are in real use); (3) a **`metadata` list column** — no page renders one yet, so `columns.*.advanced` has nothing to cut there.
...
## Sprint 63 — In-app column visibility, Tier 2: cross-device persistence + clean reset (shipped)

> **Status.** Shipped — backend PR [#107](https://github.com/9owlsboston/TagPulse/pull/107) and UI PR [#89](https://github.com/9owlsboston/TagPulse-UI/pull/89) both merged to `main` (backend-first), CI green. Backend added the merge-style **`PATCH /ui-config/me`** + granular **`DELETE /ui-config/me/columns/{page}`**, landed the **ADR-032 v1.3** amendment, and regenerated `openapi.json` (no schema change — reuses `user_ui_prefs`); `make check` green (1584). UI **retired** the Tier 1 `localStorage` layer for a server-backed `useColumnVisibility` (optimistic `PATCH` cache), added the "Reset to team default" control, adopted it on Assets + Tag Reads, and moved the `Preferences` save onto `PATCH`; `npm run check` green (478). Follow-on chore (`chore/tag-reads-epc-hex-column`) adds an `EPC (hex)` column for hex-preferring tenants. Design-doc-first — see [docs/design/configurable-column-visibility.md](design/configurable-column-visibility.md). Second of the two tiers (Tier 1 = Sprint 62, shipped).
...
## Sprint 64 — Reader location & warehouse map (shipped)

> **Status (2026-06-17, shipped).** All in-scope phases + follow-ups merged across both repos. **Design** (chore PR [#110](https://github.com/9owlsboston/TagPulse/pull/110)): [fixed-reader-positioning-and-warehouse-map.md](design/fixed-reader-positioning-and-warehouse-map.md), [tag-reads-sensor-columns.md](design/tag-reads-sensor-columns.md), [device-site-assignment-and-floor-zones.md](design/device-site-assignment-and-floor-zones.md). **Backend**: PR [#111](https://github.com/9owlsboston/TagPulse/pull/111) (Phase 0 contract — `coord_system` Site API, antenna CRUD/port-0 model, `floor_to_geo` seam, tag-reads location descriptor), [#112](https://github.com/9owlsboston/TagPulse/pull/112) (`coord_system.floorplan_image`), [#113](https://github.com/9owlsboston/TagPulse/pull/113) (device→site assignment + floor-polygon zone resolution — the *accurate* D5 path, no longer blocked). **UI**: PR [#92](https://github.com/9owlsboston/TagPulse-UI/pull/92) (Tag Reads Antenna/Temp/Humidity columns + Phase 1 placement + Phase 2 floor map), [#93](https://github.com/9owlsboston/TagPulse-UI/pull/93) (floorplan image upload + backdrop), [#94](https://github.com/9owlsboston/TagPulse-UI/pull/94) (device→site Site selector + implicit assignment on placement). Trilateration estimator stays ADR-024-deferred; the geo-anchor unified overlay stays a designed-but-unbuilt seam. `make check` green throughout (1643 final).
...
> **Status (2026-06-19, shipped — gated).** Phase 2 of [floor-position-estimation.md](design/floor-position-estimation.md) landed on backend PR [#122](https://github.com/9owlsboston/TagPulse/pull/122) across four slices: the pure estimator + `position_strategy` config, the recompute orchestration service + `FloorPositionWorker`, the TimescaleDB adapters, and the [ground-truth simulator](../scripts/simulate_floor_positioning.py). **Integration validation in dev is complete** (PRs [#124](https://github.com/9owlsboston/TagPulse/pull/124)/[#125](https://github.com/9owlsboston/TagPulse/pull/125): `simulate_floor_positioning.py --emit` seeded the survey + reads, the production pipeline wrote **8/8 computed fixes** to `asset_positions(source='computed')`, RMSE ≈ 33 m matching the offline ground-truth). The worker stays **wired in `api/main.py` but gated off** (`position_estimator_enabled`, default `False`) pending a deliberate per-environment flag-flip (ops decision). `make check` green. The `[NEEDS WM]` `rpk` wire field + v2 *snap* simulator stay deferred (now tracked under Sprint 67).
...
**Out of scope (remaining next slice).** A deliberate per-environment flip of `position_estimator_enabled` **on** (ops decision; integration validation in dev already done — see Status). Also deferred: the `[NEEDS WM]` `rpk` wire field + v2 snap simulator; per-tenant `D` cadence (the worker ticks at one base interval); per-category τ; stationary-jitter hysteresis; batching writes per tenant (the writer opens a tenant-scoped session per fix).
...
## Sprint 71 — Asset state consolidation (Phase 1, shipped)

> **Status (2026-06-20, shipped).** Shipped cross-repo (backend [#142](https://github.com/9owlsboston/TagPulse/pull/142) + UI [#108](https://github.com/9owlsboston/TagPulse-UI/pull/108)) and **validated live on dev**: opted `demo-wm-dc` in via `tenants.fusion_strategy`, flipped `CONSOLIDATION_ENABLED`, and the worker fused injected reads into an `asset_state_history` snapshot (weighted-mean temp/humidity, `sample_count`/`tag_count`) served by `GET /assets/{id}/state` (+ history). Gated **off** by default. Full design in [ADR-034](adr/034-asset-state-consolidation.md) + the [Sprint 71 design doc](design/sprint-71-asset-state-consolidation.md). **Phase 2 (transit legs) → Sprint 72.**
...
**Key insight — legs are derived, not newly ingested.** A leg is the `geo`-frame interval between two facility frames. The Phase-1 `ASSET_CUSTODY_CHANGED` events already mark the open (`facility → geo`) and close (`geo → facility`) — so Phase 2 is a thin **leg tracker** subscriber (mirroring `DwellTracker`), a table, and a read API. No new ingest, no new fusion.

**Scope (planned).**
- **`asset_legs` table** (one row per open/closed leg: origin/dest zone+site, departed/arrived, last fix, leg env envelope + SLA).
- **`AssetLegTracker`** subscribing to `ASSET_CUSTODY_CHANGED`: open on depart, close on arrive + compute the leg env envelope/SLA from `asset_state_history` over the leg window. Gated by `consolidation_enabled`; hydrates open legs on startup.
- **API** `GET /assets/{id}/legs` + the open leg on `GET /assets/{id}/state` (`open_leg` block).
- **UI** open-leg label on the "Current" card (origin + elapsed) + a Legs timeline with duration + cold-chain SLA badge.
...
**Scope.**
- **Backend:** `TagReadResponse` gains an optional `asset` ref (`AssetRef{id, name}`). `TimescaleAssetTagBindingRepository.resolve_asset_refs_by_values()` resolves it in one join (`asset_tag_bindings` → `assets`, active bindings only) across every candidate tag form (epc / epc_hex / tid / tag_id per [ADR-033](adr/033-epc-binding-resolves-uri-or-hex.md) dual-form). `QueryService` attaches it on `GET /tag-reads` and per-device `recent-reads`. No binding → `asset: null`; back-compat (no binding repo → no-op). `openapi.json` regenerated.
...
- **[ADR 024](adr/024-position-estimation.md) \u2014 Indoor position estimation (trilateration processor + `asset_positions` hypertable).** Status moved Proposed \u2192 **Deferred** in Sprint 49. Gated on first football-field-size customer asking for sub-meter `(x, y)` indoor positioning. The Sprint 41 `processor` enum is live, so the `trilateration` value can be added additively when scheduled \u2014 no schema rewrite required to unblock. **Design now captured** ([floor-position-estimation.md](design/floor-position-estimation.md), 2026-06-19) — a two-phase plan that fills the headless `asset_positions` table so an asset shows a true floor `(x, y)` + movement trail:
  ...
  - **Phase 2 — homegrown estimator** (`source='computed'`, multi-sprint, R&D): the `rssi_weighted_centroid` reference algorithm **extended with a recency-decay weight** (half-life `τ`; `τ→0` = last-one-wins), convex-hull-bounded, honest 1/2/3-antenna degradation; driven by a **server-side recompute tick (Option C)** decoupled from the wire `t=0` snap, on **server ingest time**, with the `τ` (`half_life_s`) + `D` (`recompute_interval_s`) knobs in the created-not-used `tenants.position_strategy` JSONB. Amends ADR-024 to mark the estimator implemented when it ships.
  - **`[NEEDS WM]`** Any v2 wire-format change (e.g. additive `rpk` peak-RSSI) + a v2 snap simulator are gated on WM protocol sign-off — off the estimator's critical path (Option C runs off the v1-HTTP simulators).
...
- **Migration baseline squash at v1.0.** As of Sprint 50 the repo carries 49 Alembic migrations (001–048) that encode not just DDL but ~50 `CREATE POLICY` / `ENABLE ROW LEVEL SECURITY` statements across ~22 tables, ~14 TimescaleDB calls (hypertables, continuous aggregates, compression, retention) across 8 migrations, and 4 data-seed migrations (categories, reserved `batch.*` label namespace, etc.). Pre-1.0 we considered dropping migrations entirely in favour of `Base.metadata.create_all()` + post-create hooks; estimated cost was **2–3 focused days** (baseline `schema.sql` from `pg_dump`, port RLS + Timescale DDL + seeds into post-create hooks, rewire Makefile/CI/tests, addendum to ADRs that cite migration numbers — notably ADR 028 v1.0). Decision: **keep migrations, squash at v1.0 cut.** Concrete plan: collapse 001–04X into a single `001_baseline.py` (dumped DDL + RLS + Timescale + seeds), keep Alembic, retain per-feature audit trail for everything post-baseline. Effort for squash-only path: **4–6 h**. Gated on v1.0 tag. Do NOT squash mid-flight — ADRs (028 v1.0, others) cite specific migration numbers.
