...
## Sprint 21 — Subject-Scoped Telemetry: UI & Deprecation Sunset (backend done — May 2026)
...
- [deferred to TagPulse-UI repo] **All Sprint 20 UI items** in [TagPulse-UI](https://github.com/9owlsboston/TagPulse-UI): Asset detail Telemetry tab, Lot detail Cold-chain card, Assets-list opt-in Temperature column, Devices→Telemetry `subject_kind` filter, Rules editor `subject_kind` selector + template gallery, alert detail subject context, tenant settings opt-in toggle. Backend ships every endpoint these views need.
