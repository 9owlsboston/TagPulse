...
- [planned] **B3** `[ui]` **`scripts/ui-bootstrap.sh <env>`.** Reads the four needed values (`apiFqdn`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `AZURE_STATIC_WEB_APPS_NAME`) out of the backend repo's `azd env get-values` (note: `SERVICE_API_URI` is NOT persisted by azd for the `containerapp` host — use the `apiFqdn` Bicep output and prepend `https://` to derive `VITE_API_BASE_URL`), plus the deployment token via `scripts/azd-ui-token.sh` (Phase A1 over there), and writes `.env.<env>` at mode 0600. Refuses to overwrite without `--force`. Mirrors `scripts/azd-bootstrap.sh` from this repo.
...
- [planned] **A1** `[backend]` **Pin the `/health/live` contract for SPA polling.** `/health/live` already exists (Sprint 22 A6) but is undocumented as a public surface. Sprint 25 promotes it to "polled by the SPA on cold start and on every navigation that crosses a 60s idle window". Add a contract test asserting: HTTP 200 in <50ms, no DB/MQTT touched, JSON body shape `{"status":"alive","version":"<git-sha>","build_time":"<iso8601>"}`. Add `Cache-Control: no-store` so SPA polls aren't memoized by the SWA edge.
- [planned] **A2** `[backend]` **CORS preflight max-age on `/health/*` and `/auth/login`.** The SPA fires an `OPTIONS` preflight before the first call from any new tab. Cache it for 600s via `Access-Control-Max-Age: 600` on the high-frequency endpoints; cuts 60–80ms off the first-paint-to-login-button time on cold tabs. Configurable via `Settings.cors_preflight_max_age_seconds` (default 600 in non-dev, 0 in dev so we don't fight Vite proxy quirks).
- [planned] **A3** `[backend]` **`POST /security/csp-report` endpoint.** Receives browser-issued CSP violation reports (Content-Type: `application/csp-report` or `application/reports+json` — handle both shapes). Validates the JSON envelope, emits structured logs at WARN with fields `(blocked-uri, document-uri, violated-directive, source-file, line-number, column-number, tenant_id_if_known)`, and increments a Prometheus counter `tagpulse_csp_violations_total{directive=…}`. Rate-limited at 10/min/IP via the existing rate-limiter (Sprint 22 A4) so a noisy extension doesn't DoS us. Open without auth (browsers don't send credentials on report POSTs by design). Backed by a 60-day retention policy on App Insights so we can review trends before locking the CSP down (Sprint 26+).
- [planned] **A4** `[backend]` **Update [docs/runbooks/azure-first-deploy.md](runbooks/azure-first-deploy.md) Phase 3** with the new SPA-side smoke step (curl `https://${swa}/` → response embeds `VITE_API_BASE_URL` → curl `<api>/health/ready` from the same shell as a sanity check that the SPA was built against the right api). Plus a "CSP violation triage" callout linking to the new App Insights workbook.
