...
- [done, UI PR #46 `ebb2f82`] **F7 — Asset form: drop visible `Type` field.** `src/pages/assets/AssetList.tsx` and `src/pages/assets/AssetDetail.tsx` drop the visible `asset_type` input. A hidden `Form.Item` pins `asset_type` to `'asset'` for create and preserves the existing value for edit, so the backend (still carrying the column until Phase H) keeps round-tripping cleanly. Filter chips on the Assets list page drop the legacy Type chip in favour of the existing Category chip. **Note:** the column itself is still present in the API — Phase H (the backing migration that drops the column) is deferred to a follow-up sprint because the breaking API change (`?asset_type=`, `Asset*.asset_type`) deserves its own release-note pass.
...
- [deferred to a follow-up sprint] **G5 — ADR 019 close-out.** Depends on Phase H (the schema migration that actually drops `assets.asset_type`). Phase H is deferred because the column drop is a breaking API change (removes `?asset_type=`, `Asset*.asset_type`) that deserves its own release-note pass; the UI work in Phase F7 already drops the visible field, so customers see the deprecation today even while the backend still round-trips the column. When Phase H ships, this item flips: ADR 019 Status → Completed; [docs/data-models.md](data-models.md) drops the `assets.asset_type` row + the deprecated-shadow note; [docs/design/assets-and-zones.md](design/assets-and-zones.md) + [docs/design/tracking-modes.md](design/tracking-modes.md) + [docs/guides/domain-concepts-101.md](guides/domain-concepts-101.md) drop `asset_type` from schema snippets and example payloads.

### Phase H — Close ADR 019: drop `assets.asset_type` (deferred to a follow-up sprint)

> **Status (Sprint 41 close-out):** Phase H is **deferred to a follow-up sprint**. The Phase F7 UI work already drops the visible `Type` field on Asset create/edit forms (a hidden `Form.Item` pins `asset_type` so the backend, still carrying the column, keeps round-tripping cleanly), so customers see the deprecation today. The column drop itself is a breaking API change that removes the `?asset_type=` query param + the `asset_type` field from `AssetCreate` / `AssetUpdate` / `AssetResponse` / `AssetInZoneSummary`; it deserves its own release-note pass + a one-release transitional HTTP 400 hint (per H4 below) rather than getting bundled into the Sprint 41 Unreleased section. The plan below is preserved verbatim and is the implementation plan when the follow-up sprint opens. ADR 019 close-out (Phase G5) is paired with this work and waits with it.

ADR 019 (Sprint 34) introduced `category_id` as the structured replacement for the free-form `assets.asset_type` `VARCHAR(50) NOT NULL` shadow column, with the explicit plan: "Drops in a future migration once UI + clients have switched." Sprint 41 brought the UI side (Phase F7); the follow-up sprint completes the backend half. The new Signaling Events flow scopes by category, not type, so completing the deprecation keeps the API surface internally consistent. **Breaking API change** — bumps the Unreleased section with a migration note for external webhook consumers and any direct API callers.
