...
curl "$(azd env get-value SERVICE_API_URI)/health/live"
...
| Provision fails: Mosquitto ACI `403` accessing storage | Race during first storage-key propagation | Re-run `azd provision`; second pass succeeds |
| `azd env get-value SERVICE_API_URI` is empty | API container app wasn't created (provision failed earlier) | Inspect resource list + ARM deployment errors |
