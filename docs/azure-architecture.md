...
| `Microsoft.DBforPostgreSQL/flexibleServers/databases` | `postgres.bicep` | `tagpulse` | The single application database. RLS policies declared in migrations (theatre — see [tagpulse-conventions](memories/tagpulse-conventions); explicit tenant_id WHERE everywhere). |
