...
> - **Surveyed floor plan:** give the **Site** a coordinate system (**Floor plan** button in Sites & Zones → units, extent, origin, optional floorplan image), then **drop each fixed reader onto the floor** to record its position (this writes the reader's port-0 antenna *and* assigns it to the site). Zones drawn on that floor are interpreted as **floor `(x, y)` polygons**, and a fixed read resolves to its zone by point-in-polygon of the reading antenna's position — the *accurate* path, preferred over `reader_bound`. The read-only **floor map** (Map page → pick the floor site) shows readers and, for each asset, its **floor position and movement trail** when one is available, falling back to the reader that last saw it when it isn't (see below).
>
> **Precomputed `(x, y)` positions (Sprint 65).** If you run an external location engine (vendor middleware, UWB, BLE-AoA), push resolved floor fixes to **`POST /assets/{id}/position`**; they land in `asset_positions` (`source='precomputed'`) and the floor map draws each asset's **actual position and a fading movement trail** (`GET /assets/{id}/floor-path`, dots shaded by confidence). **Homegrown sub-meter estimation** — trilateration that *computes* `(asset, x, y, confidence)` from reader RSSI — **shipped in Sprint 66 but is off by default**: an operator enables it per environment after validating accuracy against the floor survey (see [floor-position-estimation.md](design/floor-position-estimation.md) Phase 2, [ADR 024](adr/024-position-estimation.md), and the operator quickstart "Indoor positioning estimator" note). Until it's enabled, an asset **without** a precomputed fix snaps to the triggering reader rather than a computed point.
